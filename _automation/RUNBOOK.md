...
5. **Quality gate** (skip & log if fails): word count ≥ 900, has Article+FAQPage schema, single H1, links to pillar + money page, no "Memopezil", slug not duplicated, **≤ 1 em dash (—) in the whole article**. **Image gate:** file exists, is a real photograph (NOT a brain-scan/MRI/PET, diagram, chart or illustration), ≥ 40 KB, and passes the uniqueness guard. **Reject flat gradients / solid-color renders:** a smooth color gradient is NOT a photo even at ≥ 40 KB (this shipped 4 bad images once — green/white/gold/blue rectangles). Detect with an edge-energy check before saving: `python3 -c "import sys;from PIL import Image;im=Image.open(sys.argv[1]).convert('L').resize((200,200));p=list(im.getdata());d=sum(abs(p[i]-p[i-1]) for i in range(1,len(p)))/len(p);print('FLAT-REJECT' if d<3 else 'OK', round(d,2))" <file>` — if it prints `FLAT-REJECT`, discard and pick a real photo. Run `python3 _automation/check-image-unique.py --audit` at the end of the run — it must print `OK` (zero colliding groups) before you commit.
